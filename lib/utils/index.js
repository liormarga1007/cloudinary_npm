...
(function() {
  var CLOUDINARY_JS_CONFIG_PARAMS, CONDITIONAL_OPERATORS, DEFAULT_RESPONSIVE_WIDTH_TRANSFORMATION, LAYER_KEYWORD_PARAMS, PREDEFINED_VARS, TRANSFORMATION_PARAMS, URL_KEYS, at, build_custom_headers, build_eager, clone, compact, config, crc32, createSrcset, crypto, defaults, extend, filter, finalize_resource_type, finalize_source, find, first, generate_token, hashToQuery, identity, includes, isArray, isEmpty, isFunction, isNumber, isObject, isPlainObject, isString, isUndefined, join_pair, jsonArrayParam, keys, last, map, merge, norm_range_value, normalize_expression, number_pattern, offset_any_pattern, offset_any_pattern_re, process_if, process_layer, process_video_params, querystring, singleSource, smart_escape, sortBy, split_range, take, textStyle, unsigned_url_prefix, url, utf8_encode, utils, v1_adapter, v1_result_adapter,
    slice = [].slice;
  ...
  TRANSFORMATION_PARAMS = ['angle', 'aspect_ratio', 'audio_codec', 'audio_frequency', 'background', 'bit_rate', 'border', 'color', 'color_space', 'crop', 'default_image', 'delay', 'density', 'dpr', 'duration', 'effect', 'end_offset', 'fetch_format', 'flags', 'gravity', 'if', 'keyframe_interval', 'offset', 'opacity', 'overlay', 'page', 'prefix', 'quality', 'radius', 'responsive_width', 'size', 'start_offset', 'streaming_profile', 'transformation', 'underlay', 'variables', 'video_codec', 'video_sampling', 'x', 'y', 'zoom'];
  ...
  URL_KEYS = ['api_secret', 'auth_token', 'cdn_subdomain', 'cloud_name', 'cname', 'format', 'format', 'private_cdn', 'resource_type', 'secure', 'secure_cdn_subdomain', 'secure_distribution', 'shorten', 'sign_url', 'ssl_detected', 'type', 'url_suffix', 'use_root_path', 'version'];

  exports.extractConfigParams = function(options) {
    var opt;
    if (utils.isEmpty(options)) {
      return {};
    }
    opt = {};
    URL_KEYS.forEach(function(name) {
      if (options[name]) {
        return opt[name] = options[name];
      }
    });
    return opt;
  };
  ...
  singleSource = function(publicId, options, w) {
    var o, transformation;
    o = clone(options);
    transformation = utils.generate_transformation_string(o) + "/c_scale,w_${w}";
    return utils.url(publicId, extend(o, {
      raw_transformation: transformation
    })) + " ${w}w";
  };

  createSrcset = function(publicId, stops, options) {
    if (stops == null) {
      stops = [];
    }
